...
impl State {
    ...
    fn delete_oldest_logs(&self, max_files: usize) -> anyhow::Result<()> {
        ...
        // Get the prefix to match against log files
        let prefix = self.log_filename_prefix.clone().unwrap_or_default();
        ...
        let mut files = read_dir
            .filter_map(|entry| {
                ...
                if prefix.is_empty() && self.log_filename_suffix.is_none() {
